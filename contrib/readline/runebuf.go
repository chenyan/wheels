...
func (r *RuneBuffer) getBackspaceSequence() []byte {
	// 需要向左移动的列数 = 光标右侧内容的显示宽度（双宽字符如中文占 2 列）
	backWidth := runes.WidthAll(r.buf[r.idx:])
	if backWidth <= 0 {
		return nil
	}
	if r.width <= 0 {
		// 无法按行处理时，仅按列回退
		return bytes.Repeat([]byte{'\b'}, backWidth)
	}
	// 当前光标在“总列坐标”中的位置（含 prompt），取模得到当前行内列位置
	totalWidth := r.promptLen() + runes.WidthAll(r.buf)
	col := totalWidth % r.width
